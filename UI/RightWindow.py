...
class RightWindow(QWidget):
    ...
    def draw_style_trans(self, layout):
        ...
        for i, style in enumerate(styles):
            ...
            style_btn.setStyleSheet("QPushButton{background-color:rgba(100,225,100,30);\
                                    border-style:outset;\
                                    border-width:4px;\
                                    border-radius:10px;\
    ...
    def draw_object_detect(self, layout):
        ...
        detec_btn.setStyleSheet(qss)
        detec_btn.setFixedSize(120, 40)
        detec_btn.clicked.connect(self.detect)
        grid.addWidget(detec_btn, 1, 1)

        cloud_btn = QPushButton("批量读图", object_box)
        cloud_btn.setStyleSheet(qss)
        cloud_btn.setFixedSize(120, 40)
        cloud_btn.clicked.connect(self.cloud)
    ...
    def detect(self):
        self.parent().image_window.object_detect()
